...
const scriptTerminalsByScript = computed(() =>
  Object.fromEntries(
    props.health.scripts.flatMap((script) => {
      const task = taskForScript(script.name);
      const terminal = task ? props.taskTerminalsByTask[task.id] : undefined;
...
function taskForScript(scriptName: string) {
  return props.nodeTasks.find((task) => task.id === `node:${scriptName}`) ??
    props.nodeTasks.find((task) => task.name === scriptName);
}
...
function projectScriptTerminal(scriptName: string) {
  return scriptTerminalsByScript.value[scriptName];
}
...
function runNodeScript(scriptName: string) {
  const task = taskForScript(scriptName);
...
function restartNodeScript(scriptName: string) {
  const task = taskForScript(scriptName);
...
<template>
  <section class="project-health-ecosystem" aria-label="Node health">
    <div class="project-health-section-heading">
      <div>
        <span>Node</span>
        <h4>{{ nodeTasks.length }} tasks detected</h4>
      </div>
    </div>
      ...
      <div class="project-health-section-heading">
        ...
        <div class="project-health-section-actions">
          <RunProjectHealthScriptsButton
            ...
            @run-script="runNodeScript"
            @restart-script="restartNodeScript"
          />
        </div>
      </div>
